  ...
  <div class="payment-shell">
    <div v-if="!pagoSucesso">
      <div class="payment-header">
        ...
        <h3><span class="desktop-payment-title">Finalizar pagamento</span><span class="mobile-payment-title">Finalizar o agendamento</span></h3>
        <p>Conclua o agendamento com segurança. O valor fica protegido pela Seven Shows até a realização do show.</p>
      </div>

      <div class="mobile-payment-protection">
        <p>
          🔒 PAGAMENTO PROTEGIDO PELA SEVEN SHOWS<br/>
          <span>O valor permanece protegido na plataforma e o artista só recebe após a realização do show e sua liberação.</span>
        </p>
      </div>

      <div class="mobile-payment-total" aria-label="Total a pagar">
...
export default {
  name: "Step4Pagamento",
  props: { metodoPreSelecionado: { type: String, default: "pix" }, totalGeral: { type: Number, default: 0 } },
  data() { 
    return { 
...
@media(max-width:700px){.success-state{padding:28px 0 24px}.success-state h3{font-size:21px}.success-steps{padding:4px 13px;margin-top:23px}.success-step{align-items:flex-start}.success-actions button{width:100%;min-width:0}.holder-summary,.holder-fields-head{flex-direction:column;align-items:flex-start}.holder-grid{grid-template-columns:minmax(0,1fr);width:100%}.holder-grid>div,.holder-grid .holder-full{grid-column:auto;width:100%;min-width:0}.holder-grid input{display:block;width:100%!important;max-width:none!important;min-width:0;box-sizing:border-box}.pix-start-card{padding:24px 18px}.pix-benefits{display:grid;gap:7px}.payment-shell{padding:24px 18px}.payment-panel{padding:22px 16px}.copy-box{grid-template-columns:1fr}.copy-box button{padding:13px}.card-grid{grid-template-columns:1fr}.qr-frame img{width:170px;height:170px}}

.mobile-payment-title,.mobile-payment-total,.mobile-payment-protection{display:none}
@media(max-width:700px){
  .desktop-payment-title{display:none}
  .mobile-payment-title{display:inline}
  .payment-header{margin-bottom:16px}
  .payment-header .payment-kicker,.payment-header p{display:none}
  .mobile-payment-protection{display:block;background-color:rgba(10,179,156,.06);border:1px solid rgba(10,179,156,.25);border-radius:8px;padding:12px;margin:0 0 16px;text-align:center;box-shadow:inset 0 0 10px rgba(10,179,156,.02)}
  .mobile-payment-protection p{color:#0ab39c;font-size:10px;margin:0;font-weight:bold;text-transform:uppercase;letter-spacing:.3px}
  .mobile-payment-protection span{color:#c0c3cb;font-size:11px;font-weight:normal;text-transform:none;display:block;margin-top:4px;line-height:1.4}
  .mobile-payment-total{display:flex;flex-direction:column;gap:4px;margin:0 0 16px;padding:15px 16px;border:1px solid rgba(255,108,34,.28);border-radius:11px;background:rgba(255,108,34,.055)}
  .mobile-payment-total span{font-size:9px;font-weight:900;letter-spacing:.8px;color:#9da3b2}
  .mobile-payment-total strong{font-size:22px;color:#ff6c22;line-height:1.2}
  .pix-panel{padding:0;background:transparent;border:0}
  .pix-start-card{padding:20px 16px 18px}
  .pix-start-card .method-icon{width:46px;height:46px;margin-bottom:10px}
  .pix-start p{margin-bottom:14px}
  .pix-benefits{display:none!important}
  .pix-start small{margin-top:10px}
}
</style>
